...
const Join =()=>{
    ...
    // const [loginIDDuplicate, loginIDDuplicateResult] = useMutation(
    //   LOGINID_DUPLICATE,
    // );
  
    // const [emailDuplicate, emailDuplicateResult] = useMutation(EMAIL_DUPLICATE);

    const [signupDone, setSignupDone] = useState<boolean>(false);
    const [isIdDuplicate, setSsIdDuplicate] = useState<boolean>(false);
    ...
    useEffect(() => {
    //   if (loginIDDuplicateResult.loading) return;
  
    //   if (loginIDDuplicateResult?.data?.loginIdDuplicate) {
    ...
    useEffect(() => {
    //   if (emailDuplicateResult.loading) return;
  
    //   if (emailDuplicateResult?.data?.emailDuplicate) {
    //     setEmailValid(false);
    //   } else {
    //     setEmailValid(true);
    //   }
    ...
    const signup = async (
        ...
        try {
            ...
            setJoinValue({...joinValue, password:hashedPassword})
            //params는 어케쓰지
            //여기도 수정좀 해야될듯
      ...
      const idDuplicate = async (inputId : string)=>{
    ...
    const emailCheck = (email:any) => {
      ...
      let isValidEmail = email !== '' && email !== 'undefined' && regex.test(email);
