...
def get_project_detail_data(project_id: int):
    query = f"""
        SELECT p.project_name, FORMAT(ROUND(p.building_area, 2), 2) AS building_area, p.construction_company, 
        p.location, FORMAT(ROUND(p.total_area, 2), 2) AS total_area, p.construction_start, p.construction_end,
