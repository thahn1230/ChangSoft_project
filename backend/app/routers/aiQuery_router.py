...
from .insight_router import get_insight_1, get_insight_2, get_insight_3, get_insight_4, get_insight_5, get_insight_6
...
async def execute_query(query: Query): 
    ...
    if(mode == "Insight 1"):
        plot_data = get_insight_1("[2, 3]")
        return generate_explanation_byplot(plot_data)
    elif(mode == "Insight 2"):
        plot_data = get_insight_2("[2, 3]")
        return generate_explanation_byplot(plot_data)
    elif(mode == "Insight 3"):
        plot_data = get_insight_3("[2, 3]")
        return generate_explanation_byplot(plot_data)
    elif(mode == "Insight 4"):
        plot_data = get_insight_4("[\"우미건설\", \"계룡건설\"]")
        return generate_explanation_byplot(plot_data)
    elif(mode == "Insight 5"):
